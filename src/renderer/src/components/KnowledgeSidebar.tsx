...
// FiBookOpen is used in other components importing this file
import { useIntl } from '../lib/locale'

export function MdiBrain(): React.JSX.Element {
  return (
...
export function LetsIconsNotebook(): React.JSX.Element {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 24 24">
      {/* Icon from Lets Icons by Leonid Tsvetkov - https://creativecommons.org/licenses/by/4.0/ */}
      <g fill="none" stroke="currentColor" strokeWidth="2">
        <rect width="13" height="17" x="6" y="4" rx="2"></rect>
        <path strokeLinecap="round" d="M15 10V8M4 9h4m-4 4h4m-4 4h4"></path>
      </g>
    </svg>
  )
}
...
function KnowledgeSidebar({ isOpen }: KnowledgeSidebarProps): React.JSX.Element | null {
  const [summary, setSummary] = useState<Array<{ title: string; content: string }> | string>([])
  const [isLoading, setIsLoading] = useState(false)
  const intl = useIntl()

  const fetchSummary = useCallback(async (): Promise<void> => {
    if (summary.length > 0 && !isLoading) return

    setIsLoading(true)
    try {
      const response = await window.api.summarize()
      setSummary(response)
    } catch {
      setSummary(`Error fetching knowledge summary.`)
    } finally {
      setIsLoading(false)
    }
  }, [summary, isLoading])
      ...
      <div className="sidebar-header">
        <div className="sidebar-text">
          <div className="sidebar-title">
            {intl.formatMessage({ id: 'knowledge' }) || 'Knowledge'}
          </div>
        </div>
      </div>
