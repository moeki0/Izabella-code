...
describe('database', () => {
  ...
  it('すべてのテーブルとインデックスの作成クエリが実行されること', async () => {
    ...
    // すべてのクエリが実行されたか確認
    expect(mockExec).toHaveBeenNthCalledWith(1, expect.stringContaining('CREATE TABLE IF NOT EXISTS threads'))
    expect(mockExec).toHaveBeenNthCalledWith(2, expect.stringContaining('CREATE TABLE IF NOT EXISTS messages'))
    expect(mockExec).toHaveBeenNthCalledWith(3, expect.stringContaining('CREATE TABLE IF NOT EXISTS workflows'))
    expect(mockExec).toHaveBeenNthCalledWith(4, expect.stringContaining('CREATE INDEX IF NOT EXISTS idx_messages_thread_id'))
    expect(mockExec).toHaveBeenNthCalledWith(5, expect.stringContaining('CREATE INDEX IF NOT EXISTS idx_workflows_title'))
  })
  ...
  it('messagesテーブルが期待される構造で作成されること', async () => {
    ...
    expect(messagesQuery).toContain('FOREIGN KEY (thread_id) REFERENCES threads (id) ON DELETE CASCADE')
  })
