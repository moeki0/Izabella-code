...
~~~
<reasoning>
この会話では重要なトピックが扱われたのでナレッジに保存します。まずはsearch_knowledgeを使って既存のナレッジを検索します。その後upsert_knowledgeツールを利用し先ほどのナレッジを更新します。すでにあるナレッジを破壊しないように情報を統合します。そして、ツールを使ってナレッジを保存した後にupdate_memoryを使ってナレッジインデックスを更新します。
</reasoning>
...
~~~
<reasoning>
この会話でトピックスが完了したとみなされるのでナレッジにこの結果を保存します。まずはsearch_knowledgeを使って既存のナレッジを検索します。その後upsert_knowledgeツールを利用し先ほどのナレッジを更新します。すでにあるナレッジを破壊しないように情報を統合します。そして、ツールを使ってナレッジを保存した後にupdate_memoryを使ってナレッジインデックスを更新します。
</reasoning>
